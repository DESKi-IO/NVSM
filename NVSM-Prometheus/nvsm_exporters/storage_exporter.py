...
def init():
    ...
    if data_ssd_count not in metric_map:
        metric_map[data_ssd_count] = Gauge(
            data_ssd_count, "Number of Data Drives")

    if health not in metric_map:
        metric_map[health] = Gauge(health, "Drive Health")

    if avg_data not in metric_map:
        metric_map[avg_data] = Gauge(
            avg_data, "Average Percent used Data Drives")
...
def ExportMetric(ip="localhost", port="273"):
    ...
    r = requests.get(
        'https://' + str(ip) + ':' + str(port) + '/redfish/v1/Systems/1/Storage',
        timeout=120,
        verify=False,
        headers={
    ...
    for data_id in storage_collection["Members"]:
        ...
        # Request to URL to get the data
        r = requests.get(
            'https://' + str(ip) + ':' + str(port) + '/' + data_id["@odata.id"],
            timeout=120,
            verify=False,
            headers={
                'Authorization': 'Bearer ' + tokenstring})
        ...
        for nvme_id in nvme_storage_subsys["Drives"]:
            ...
            r = requests.get(
                'https://' + str(ip) + ':' + str(port) + nvme_id["@odata.id"],
                timeout=120,
                verify=False,
                headers={
                    'Authorization': 'Bearer ' + tokenstring})
            ...
            if "nvme0n1" in drive["@odata.id"] or "nvme1n1" in drive["@odata.id"]:
                ...
                avg_os_used += float(drive["Oem"]["Nvidia_HM"]
                                     ["Metrics"]["PercentUsed"])
            else:
                usage_name = "data_" + \
                    drive["@odata.id"][-7:] + "_percent_used"
                avg_data_used += float(drive["Oem"]
                                       ["Nvidia_HM"]["Metrics"]["PercentUsed"])
            ...
            if "nvme0n1" in drive["@odata.id"] or "nvme1n1" in drive["@odata.id"]:
                ...
                error_name = "data_" + \
                    drive["@odata.id"][-7:] + "_media_errors"
